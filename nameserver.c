...
void update_name(int sockfd, int dbfd, struct name_prtl *name_request);
        ...
        if (result == -1) { /* fail */
            ...
            if (m == -1) {
                ...
                if (name_request->type == 2) { /* add new name */
                    ...
                    update_name(sockfd, dbfd, name_request);
                }
            }
...
void name_server(int sockfd, int dbfd, int itemfd, int rservfd, int port)
{
    ...
    if (buf[0] == '1') {
        ...
        if (name_request.type > 4 || name_request.type <= 0) {
            ...
            handle_request(&name_request, itemfd, dbfd, rservfd, sockfd, n, 
                port, data);
        }
